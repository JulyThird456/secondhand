  ...
  <div class="dashboard-container">
    <div class="container">
      <div class="nav">
        ...
        <span class="title">{{ isEdit ? '编辑套餐' : '添加套餐' }}</span>
      </div>
...
import { addSetmeal, editSetmeal, querySetmealById, dishCategoryList } from '@/api/setMeal'
...
const form = reactive({ name: '', categoryId: '', price: 0 })
...
onMounted(async () => {
  const { data } = await dishCategoryList({ type: 2 })
  if (data.code === 1) categoryOptions.value = data.data
  if (route.query.id) {
    const res = await querySetmealById(route.query.id as string)
...
async function submitForm() {
  formRef.value?.validate(async (valid) => {
    if (!valid) return
    try {
      const res = isEdit.value ? await editSetmeal(form) : await addSetmeal(form)
